...
(buf)[(len) - i - 1] = tmp; \
    ...
    static inline Range _CHDataFindBytes(const char *data,
...
{
    return StringSearch::_CHDataFindByteUsually(src, srcLen, pattern, patternLength);
}

void searchAllOfOccurrencesOfString(const char *src, uint32_t srcLen, const char *pattern, uint32_t patternLength, vector<Range> &result)
{
